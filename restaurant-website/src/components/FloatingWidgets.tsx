...
import { ArrowUp, Utensils, MessageSquare, Send, X, CheckCheck } from "lucide-react";
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (userMsg.toLowerCase().includes("reserve") || userMsg.toLowerCase().includes("table") || userMsg.toLowerCase().includes("book")) {
        ...
        replyText = "Our signature menu features crispy fried chicken, special cheeseburgers, wood-fired pizzas, and rolls. You can view all details under the 'Menu' section.";
      }
    ...
    <div className="fixed bottom-6 right-6 z-40 flex flex-col items-center gap-3">
      ...
      <AnimatePresence>
        {showWhatsAppChat && (
          <motion.div
            ...
            className={`w-80 sm:w-96 rounded-2xl shadow-2xl border overflow-hidden flex flex-col mb-4 ${theme === "dark"
          ...
          >
            ...
            <div className="bg-gradient-to-r from-gold-600 to-gold-500 p-4 text-stone-950 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="font-serif font-bold tracking-wide">Frydaddy Assist</h4>
                  <p className="text-[10px] uppercase font-semibold text-stone-850 dark:text-stone-300">Virtual Assistant</p>
                </div>
              </div>
              <button
            ...
            <form onSubmit={handleSendMessage} className="p-3 border-t dark:border-stone-800 border-stone-200 flex gap-2">
              <input
                ...
                placeholder="Ask about burgers, fried chicken..."
                className={`flex-grow px-3 py-2 text-sm rounded-lg border outline-none bg-transparent ${theme === "dark"
      ...
      <div className="flex flex-col gap-3">
        ...
        <button
          ...
          className={`flex items-center justify-center w-12 h-12 rounded-full border shadow-xl hover:scale-110 hover:-translate-y-1 transition-all duration-300 cursor-pointer ${showWhatsAppChat
        ...
        <AnimatePresence>
          {showScrollTop && (
            <motion.button
              ...
              onClick={scrollToTop}
              className={`flex items-center justify-center w-12 h-12 rounded-full border shadow-xl hover:scale-110 hover:-translate-y-1 transition-all duration-300 cursor-pointer ${theme === "dark"
